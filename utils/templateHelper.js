...
function buildLink(longname, linkText, options) {
  ...
  // (but skip anything that looks like an inline tag or HTML tag)
    ...
    fileUrl = hasOwnProp.call(options.linkMap, longname)
      ...
      : '';
    text =
      linkText || (options.shortenName ? getShortName(longname) : longname);
  }
 ...
 */
const linkto = (exports.linkto = (key, longname, linkText, cssClass, fragmentId) =>
...
const sectionToUrl = (exports.sectionToUrl = (key, section) => {
  let fileUrl;
  const node = sections[key].getByName(section);

  // no such tutorial
  if (!node) {
    logger.error(new Error(`No such section ${key}: ${section}`));
  ...
  // define the URL if necessary
  if (!hasOwnProp.call(tutorialLinkMap.nameToUrl, node.name)) {
    fileUrl = `${key}-${getUniqueFilename(node.name)}`;
    tutorialLinkMap.nameToUrl[node.name] = fileUrl;
    tutorialLinkMap.urlToName[fileUrl] = node.name;
  }
...
const toSection = (exports.toSection = (key, section, content, missingOpts) => {
  let classname;
  let link;
  let node;
  let tag;
  ...
  if (!section) {
    logger.error(new Error(`Missing required parameter: section ${key}`));

    return null;
  }

  node = sections[key].getByName(section);
  // no such tutorial
  if (!node) {
    missingOpts = missingOpts || {};
    tag = missingOpts.tag;
  ...
  return `<a href="${sectionToUrl(key, section)}">${content}</a>`;
});
...
exports.resolveLinks = (str) => {
  ...
  function processSection(key, string, { completeTag, text }) {
    const leading = extractLeadingText(string, completeTag);

    string = leading.string;

    return string.replace(completeTag, toSection(key, text, leading.leadingText));
  }
...
exports.getMembers = (data) => {
  ...
  members.globals = members.globals.filter(
    (doclet) => !isModuleExports(doclet)
  );
...
exports.getAncestorLinks = (data, doclet, cssClass) => {
  ...
  ancestors.forEach((ancestor) => {
    const linkText =
      (exports.scopeToPunc[ancestor.scope] || '') + ancestor.name;
